...
def draw_camera_boards(ax, camera_matrix, cam_width, cam_height, scale_focal,
    ...
    for idx in range(extrinsics.shape[0]):
        cMo = np.eye(4, 4)
        R = extrinsics[idx, :, :3]  # First three columns
        T = extrinsics[idx, :, 3]  # Last three columns
        cMo[0:3, 0:3] = R
        cMo[0:3, 3] = T
        for i in range(len(X_moving)):
            ...
            if not patternCentric:
                X[1, :] *= -0.2 # Arbitary number(I couldn't find the source of the error)
            ax.plot3D(X[0,:], X[1,:], X[2,:], color=colors[idx])
            min_values = np.minimum(min_values, X[0:3,:].min(1))
            max_values = np.maximum(max_values, X[0:3,:].max(1))
        ax.text(X[0, 0] + 1, X[1, 0] + 1, X[2, 0] + 1, f"Cam {idx + 1}", color='red', fontsize=10)
